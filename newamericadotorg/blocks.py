...
class CustomImageBlock(blocks.StructBlock):
    ...
    use_original = blocks.BooleanBlock(
        required=False, help_text="check if you do not want image compressed. Should be checked for all figures.")
...
class IframeBlock(blocks.StructBlock):
    ...
    width = IntegerBlock(
        max_value=1050, help_text="The maximum possible iframe width is 1050")
    height = IntegerBlock()
...
class DatavizBlock(blocks.StructBlock):
    ...
    max_width = IntegerBlock(
        help_text='for legacy dataviz projects', required=False)
...
def getJSCompatibleList(input_list, is_era, sort):
    if sort:
        sortedList = sorted(
            input_list, key=lambda member: member['start_date'])
    else:
...
class TimelineBlock(blocks.StructBlock):
    ...
    event_eras = blocks.ListBlock(
        TimelineEraBlock(), default='', required=False)
    event_categories = blocks.ListBlock(
        blocks.CharBlock(), default='', required=False)
    event_list = blocks.ListBlock(TimelineEventBlock())

    def get_context(self, value, parent_context=None):
        context = super(TimelineBlock, self).get_context(
            value, parent_context=parent_context)
        context["sorted_event_list"] = sorted(
            value["event_list"], key=lambda member: member['start_date'])
        context["settings_json"] = json.dumps({"eventList": getJSCompatibleList(value["event_list"], False, True), "defaultView": value["default_view"], "eraList": getJSCompatibleList(
            value["event_eras"], True, True), "splitList": getJSCompatibleList(value["major_timeline_splits"], True, False), "categoryList": value["event_categories"]})
        return context
...
class GoogleMapBlock(blocks.StructBlock):
    use_page_address = blocks.BooleanBlock(
        default=False, required=False, help_text="If selected, map will use the address already defined for this page, if applicable. For most posts besides events, this should be left unchecked and the form below should be completed.")
...
class SessionDayBlock(blocks.StructBlock):
    collapsible = blocks.BooleanBlock(
        help_text="Allow schedule sessions to expand and collapse", required=False, default=True)
    day = IntegerChoiceBlock(
        help_text="What day of the conference is this session on?", required=False, default=1)
    start_time = blocks.TimeBlock(required=False)
    end_time = blocks.TimeBlock(required=False)
...
class RandomThoughtBlock(blocks.StructBlock):
    thoughts = blocks.StreamBlock([
        ('thought', blocks.StructBlock([
            ('name', blocks.TextBlock(required=False)),
            ('image', ImageChooserBlock(icon="image", required=False)),
            ('title', blocks.CharBlock(required=False)),
...
class Body(blocks.StreamBlock):
    ...
    heading = blocks.CharBlock(
        classname='full title', icon="title", template="blocks/heading.html")
    ...
    random_thought = RandomThoughtBlock()
...
class BodyBlock(Body):
    ...
    people = PeopleBlock(
        help_text="Grid of people with short bios that appear on click", icon="group")
    panels = PanelsBlock(icon="list-ul")
    image = ImageChooserBlock(template='blocks/image_block.html',
                              help_text='Legacy option. Consider using Inline Image instead.', icon="placeholder")
